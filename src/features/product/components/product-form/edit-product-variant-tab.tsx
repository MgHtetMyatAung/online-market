...
interface VariantsTabProps {
  ...
  handleVariantCheckboxChange: (idx: string) => void;
...
export const EditVariantsTab: React.FC<VariantsTabProps> = ({
  ...
  const chooseAllVariants = () => {
    if (checkAll) {
      ...
      setCheckAll(true);
      const allVariantIndex = variantFields.map((item) => item.sku);
      handleSelectAllVariants(allVariantIndex);
    }
  };
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    ...
                    {variantFields.map((field, index) => (
                      <div
                        key={field.id}
                        className={clsx(
                          "p-3 border-t grid grid-cols-12 gap-2 items-center",
                          {
                            " bg-blue-100": selectedIndex.includes(field.sku),
                          },
                        )}
                      >
                        <div className="col-span-3 flex gap-3">
                          <input
                            type="checkbox"
                            checked={selectedIndex.includes(field.sku)}
                            onChange={() =>
                              handleVariantCheckboxChange(field.sku)
                            }
                            className="rounded border-gray-300"
                          />
                          <Input
